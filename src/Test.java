public class Test {
    ...
    static void testMaxIndex(int[] row, int start, int end, int output) {
        System.out.println("Testing maxIndex on row between " + start + " and " +
                end);
        int result = Solution.maxIndex(row, start, end);
        printTest(result, output);
    }
    static void testBlockMaxValue(int[][] matrix, int startRow, int startCol, int
            endRow, int endCol, int output) {
        System.out.println("Testing blockMaxValue between (" + startRow + "," +
                startCol + ") and (" + endRow + "," + endCol + ")");
        int result = Solution.blockMaxValue(matrix, startRow, startCol, endRow,
                endCol);
        printTest(result, output);
    }
